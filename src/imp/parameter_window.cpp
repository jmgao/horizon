...
namespace horizon {
ParameterWindow::ParameterWindow(Gtk::Window *p, std::string *ppc, ParameterDef *pd, ParameterSet *ps,
                                 ParameterSetEditor *editor)
    : Gtk::Window()
{
    set_transient_for(*p);
    ...
    if (editor)
        ...
        parameter_set_editor = Gtk::manage(new ParameterSetEditor(pd, ps));
